...
/**
 * Creates field path for a given item
 *
 * @api public
 */

Field.prototype.itemPath = function(item) {

    if (this.list.parentField){
        if (item._id === undefined) {item._id = keystone.mongoose.Types.ObjectId();}

        return this._path.itemPath(this.list.parentField.path, item._id);
