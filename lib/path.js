...
exports = module.exports = function Path(str) {
	...
	this.prependToLast = function(prepend, titlecase) {
		var rtn = '';
        // TODO: replace loop by exceptLast.join('.')+'.' ?
		for (var i = 0; i < parts.length - 1; i++) {
			rtn += parts[i] + '.';
		}
    ...
    this.bracketize = function() {
        return '[' + parts.join('][') + ']';
    };

    this.itemPath = function (prefix, itemIdx) {
        return prefix + '[' + itemIdx + '][' + str + ']';
        //return prefix + '[' + itemIdx + ']' + this.bracketize();
        //return [prefix, itemIdx, str].join('.');
    };
